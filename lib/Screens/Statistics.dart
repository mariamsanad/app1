        ...
        children: [
          FutureBuilder(
              ...
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
                  if (snapshot.data == "I am connected to a wifi network." ||
                      snapshot.data == "I am connected to a mobile network.") {
                    return Visibility(
                        visible: false, child: Text(snapshot.data));
                  }
                ...
                if (snapshot.data.length > 0) {
                  if (_showing == 'week') {
                    ...
                    for (int i = snapshot.data.length - 7;
                        ...
                        i++) {
                      if (i != 0)
                        confirmedRec.add(new DeathRec(
                            DateFormat('d-MMM-yy').format(
                                DateTime.parse(snapshot.data[i]['Date'])),
                            (snapshot.data[i]['Confirmed'] -
                                snapshot.data[i - 1]['Confirmed'])));
                      else
                        confirmedRec.add(new DeathRec(
                            DateFormat('d-MMM-yy').format(
                                DateTime.parse(snapshot.data[i]['Date'])),
                    ...
                    for (int i = snapshot.data.length - 7;
                        ...
                        i++) {
                      if (i == 410)
                        activeRec.add(new DeathRec(
                            DateFormat('d-MMM-yy').format(
                                DateTime.parse(snapshot.data[i]['Date'])),
                            snapshot.data[i - 1]['Active']));
                      else
                        activeRec.add(new DeathRec(
                            DateFormat('d-MMM-yy').format(
                                DateTime.parse(snapshot.data[i]['Date'])),
                    ...
                    for (int i = snapshot.data.length - 365;
                        i < snapshot.data.length;
                        i++) {
                      if (i != 0)
                        deathRec.add(new DeathRec(
                    ...
                    for (int i = snapshot.data.length - 30;
                        i < snapshot.data.length;
                        i++) {
                      if (i != 0)
                        deathRec.add(new DeathRec(
                  ...
                  return Expanded(
                      child: SfCartesianChart(
                          ...
                          title: ChartTitle(
                              text: '${_currentCity} chart'), //Chart title.
                          legend:
                              Legend(isVisible: true), // Enables the legend.
      ...
      if (snapshot.hasData) {
        if (snapshot.data.length > 0) {
          return CountryDetails(
            date: snapshot.data[snapshot.data.length - 1]['Date'],
            code: snapshot.data[snapshot.data.length - 1]['CountryCode'],
            confirmed: (snapshot.data[snapshot.data.length - 1]['Confirmed'] -
                snapshot.data[snapshot.data.length - 3]['Confirmed']),
            deaths: (snapshot.data[snapshot.data.length - 1]['Deaths'] -
                snapshot.data[snapshot.data.length - 3]['Deaths']),
            recoverd: (snapshot.data[snapshot.data.length - 1]['Recovered'] -
                snapshot.data[snapshot.data.length - 3]['Recovered']),
            active: snapshot.data[snapshot.data.length - 1]['Active'],
            country: snapshot.data[snapshot.data.length - 1]['Country'],
          );
        } else {
          return Expanded(
              child: Center(
